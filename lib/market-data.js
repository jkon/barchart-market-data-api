 ...
 * @param {String} key API key provided by Barchart
 */
const MarketData = function (key) {
  apiKey = key;
};
...
module.exports = {
  /**
   * Set the api key for use in the calls to Barcharts APIs
   * @param {String} key api key from barchart
   * @see https://barchart.com/ondemand/free-market-data-api for a key to the free APIs or contact barchart to register
   * for the paid APIs
   * */
  setApiKey: (key) => {
    apiKey = key;
   ...
   */
  getHistory: (symbol, type, options) => {
    type = type || 'daily';
    if (!symbol) {
